...
static char *ehbx_msg = NULL;
static size_t ehbx_msg_alloc=0, ehbx_msg_offset=0;
...
#define PACKED_FIELD(value, size, type)	\
		(zbx_packed_field_t){(value), (size), (0 == (size) ? PACKED_FIELD_STRING : type)};
	...
	for (i = 0; i < count; i++)
	{
		...
		if (NULL != offset)
		{
			...
			if (PACKED_FIELD_STRING == fields[i].type)
			{
				...
				zbx_snprintf_alloc(&ehbx_msg,&ehbx_msg_alloc, &ehbx_msg_offset, "(strlen:%d)", field_size);
				if (0 != field_size && NULL != fields[i].value){
					memcpy(offset + sizeof(zbx_uint32_t), fields[i].value, field_size);
					zbx_snprintf_alloc(&ehbx_msg, &ehbx_msg_alloc, &ehbx_msg_offset, "(string:");
					for(j=0;j<field_size;j++)
					   zbx_chrcpy_alloc(&ehbx_msg,&ehbx_msg_alloc,&ehbx_msg_offset,*(unsigned char*)(fields[i].value+j));
					zbx_chrcpy_alloc(&ehbx_msg, &ehbx_msg_alloc, &ehbx_msg_offset, ')');
				}
				field_size += sizeof(zbx_uint32_t);
			} else {
				memcpy(offset, fields[i].value, field_size);
				if (fields[i].type=='C'){
					if (*(unsigned char*)fields[i].value != '\0' && fields[i].value != NULL ){
						zbx_snprintf_alloc(&ehbx_msg,&ehbx_msg_alloc,&ehbx_msg_offset,"(uchar:");
						zbx_snprintf_alloc(&ehbx_msg, &ehbx_msg_alloc, &ehbx_msg_offset, "%d", *(unsigned char*)fields[i].value);
						zbx_chrcpy_alloc(&ehbx_msg, &ehbx_msg_alloc, &ehbx_msg_offset, ')');
					} else {
						zbx_snprintf_alloc(&ehbx_msg,&ehbx_msg_alloc,&ehbx_msg_offset,"(uchar:)");
					}
				}
				else if (fields[i].type=='I')
					zbx_snprintf_alloc(&ehbx_msg,&ehbx_msg_alloc,&ehbx_msg_offset,"(int:%d)",*(int *)fields[i].value);
				else if (fields[i].type=='L')
					zbx_snprintf_alloc(&ehbx_msg, &ehbx_msg_alloc, &ehbx_msg_offset, "(int64:%d)",*(uint64_t *)fields[i].value);
				else if (fields[i].type=='D')
					zbx_snprintf_alloc(&ehbx_msg, &ehbx_msg_alloc, &ehbx_msg_offset, "(double:%d)",*(double *)fields[i].value);
				else if (fields[i].type=='B'){
					zbx_snprintf_alloc(&ehbx_msg, &ehbx_msg_alloc, &ehbx_msg_offset, "(bin of len %d:",*(uint32_t *)fields[i].value);
					for(j=4;j<field_size;j++)
						zbx_snprintf_alloc(&ehbx_msg, &ehbx_msg_alloc, &ehbx_msg_offset, "%02x ",*(unsigned char *)(fields[i].value+j));
					zbx_chrcpy_alloc(&ehbx_msg, &ehbx_msg_alloc, &ehbx_msg_offset,')');
				} else 
					zbx_snprintf_alloc(&ehbx_msg, &ehbx_msg_alloc, &ehbx_msg_offset, "(unknown)");
			}
...
static zbx_uint32_t	preprocessor_pack_value(zbx_ipc_message_t *message, zbx_preproc_item_value_t *value)
{
	...
	unsigned char		ts_marker, result_marker, log_marker;
        zbx_uint32_t ehbx_sz;

	ts_marker = (NULL != value->ts);
	result_marker = (NULL != value->result);

	*offset++ = PACKED_FIELD(&value->itemid, sizeof(zbx_uint64_t), 'L');
	*offset++ = PACKED_FIELD(&value->item_value_type, sizeof(unsigned char), 'C');
	*offset++ = PACKED_FIELD(&value->item_flags, sizeof(unsigned char),'C');
	*offset++ = PACKED_FIELD(&value->state, sizeof(unsigned char),'C');
	*offset++ = PACKED_FIELD(value->error, 0,'S');
	*offset++ = PACKED_FIELD(&ts_marker, sizeof(unsigned char),'C');

	if (NULL != value->ts)
	{
		*offset++ = PACKED_FIELD(&value->ts->sec, sizeof(int), 'I');
		*offset++ = PACKED_FIELD(&value->ts->ns, sizeof(int), 'I');
	}

	*offset++ = PACKED_FIELD(&result_marker, sizeof(unsigned char),'C');

	if (NULL != value->result)
	{

		*offset++ = PACKED_FIELD(&value->result->lastlogsize, sizeof(zbx_uint64_t),'L');
		*offset++ = PACKED_FIELD(&value->result->ui64, sizeof(zbx_uint64_t),'L');
		*offset++ = PACKED_FIELD(&value->result->dbl, sizeof(double),'D');
		*offset++ = PACKED_FIELD(value->result->str, 0, 'S');
		*offset++ = PACKED_FIELD(value->result->text, 0, 'S');
		*offset++ = PACKED_FIELD(value->result->msg, 0, 'S');
		*offset++ = PACKED_FIELD(&value->result->type, sizeof(int), 'I');
		*offset++ = PACKED_FIELD(&value->result->mtime, sizeof(int), 'I');

		log_marker = (NULL != value->result->log);
		*offset++ = PACKED_FIELD(&log_marker, sizeof(unsigned char), 'C');
		if (NULL != value->result->log)
		{
			*offset++ = PACKED_FIELD(value->result->log->value, 0, 'S');
			*offset++ = PACKED_FIELD(value->result->log->source, 0, 'S');
			*offset++ = PACKED_FIELD(&value->result->log->timestamp, sizeof(int), 'I');
			*offset++ = PACKED_FIELD(&value->result->log->severity, sizeof(int), 'I');
			*offset++ = PACKED_FIELD(&value->result->log->logeventid, sizeof(int), 'I');
		}
	}
        
	zbx_free(ehbx_msg);ehbx_msg_alloc=0;ehbx_msg_offset=0;
	ehbx_sz = message_pack_data(message, fields, offset - fields);
	zabbix_log(LOG_LEVEL_DEBUG,"%s() packed data: code:2(preproc_request) size:%d data(itemid-valueType-flags-state-error-tsMarker-..-resultMarker-...............):%s",__func__,ehbx_sz,ehbx_msg);
...
static int	preprocessor_pack_variant(zbx_packed_field_t *fields, const zbx_variant_t *value)
{
	...
	fields[offset++] = PACKED_FIELD(&value->type, sizeof(unsigned char), 'C');

	switch (value->type)
	{
		case ZBX_VARIANT_UI64:
			fields[offset++] = PACKED_FIELD(&value->data.ui64, sizeof(zbx_uint64_t), 'L');
			break;

		case ZBX_VARIANT_DBL:
			fields[offset++] = PACKED_FIELD(&value->data.dbl, sizeof(double), 'D');
			break;

		case ZBX_VARIANT_STR:
			fields[offset++] = PACKED_FIELD(value->data.str, 0, 'S');
			break;

		case ZBX_VARIANT_BIN:
			fields[offset++] = PACKED_FIELD(value->data.bin, sizeof(zbx_uint32_t) +
					zbx_variant_data_bin_get(value->data.bin, NULL), 'B');
			break;
	}
		...
		const int *history_num)
{
	...
	fields[offset++] = PACKED_FIELD(history_num, sizeof(int), 'I');

	for (i = 0; i < *history_num; i++)
	{
		zbx_preproc_op_history_t	*ophistory = (zbx_preproc_op_history_t *)history->values[i];

		fields[offset++] = PACKED_FIELD(&ophistory->index, sizeof(int), 'I');
		offset += preprocessor_pack_variant(&fields[offset], &ophistory->value);
		fields[offset++] = PACKED_FIELD(&ophistory->ts.sec, sizeof(int), 'I');
		fields[offset++] = PACKED_FIELD(&ophistory->ts.ns, sizeof(int), 'I');
	}
...
static int	preprocessor_pack_step(zbx_packed_field_t *fields, const zbx_preproc_op_t *step)
{
	...
	fields[offset++] = PACKED_FIELD(&step->type, sizeof(char), 'C');
	fields[offset++] = PACKED_FIELD(step->params, 0, 'S');
	fields[offset++] = PACKED_FIELD(&step->error_handler, sizeof(char), 'C');
	fields[offset++] = PACKED_FIELD(step->error_handler_params, 0, 'S');
...
static int	preprocessor_pack_steps(zbx_packed_field_t *fields, const zbx_preproc_op_t *steps, const int *steps_num)
{
	...
	fields[offset++] = PACKED_FIELD(steps_num, sizeof(int), 'I');
		...
		const zbx_preproc_op_t *steps, int steps_num)
{
	...
	*offset++ = PACKED_FIELD(&itemid, sizeof(zbx_uint64_t), 'L');
	*offset++ = PACKED_FIELD(&value_type, sizeof(unsigned char), 'C');
	*offset++ = PACKED_FIELD(&ts_marker, sizeof(unsigned char), 'C');

	if (NULL != ts)
	{
		*offset++ = PACKED_FIELD(&ts->sec, sizeof(int), 'I');
		*offset++ = PACKED_FIELD(&ts->ns, sizeof(int), 'I');
	}
    ...
    zbx_free(ehbx_msg);ehbx_msg_alloc = ehbx_msg_offset=0;
	size = message_pack_data(&message, fields, offset - fields);
	zabbix_log(LOG_LEVEL_DEBUG,"%s() packed data: code:2(preproc_request) size:%d data(itemid-valueType-tsMarker-..-[value]-[history]-[steps]):%s", __func__, size, ehbx_msg);
		...
		const zbx_vector_ptr_t *history, char *error)
{
	...
	*offset++ = PACKED_FIELD(error, 0, 'S');

	zbx_ipc_message_init(&message);
	zbx_free(ehbx_msg);ehbx_msg_alloc=ehbx_msg_offset=0;
	size = message_pack_data(&message, fields, offset - fields);
	zabbix_log(LOG_LEVEL_DEBUG,"%s() packed data: code:3(preproc_result) size:%d data([value]-[history]-error):%s",__func__,size,ehbx_msg);
		...
		int results_num, const zbx_vector_ptr_t *history, const char *error)
{
	...
	*offset++ = PACKED_FIELD(&results_num, sizeof(int), 'I');

	for (i = 0; i < results_num; i++)
	{
		offset += preprocessor_pack_variant(offset, &results[i].value);
		*offset++ = PACKED_FIELD(results[i].error, 0, 'S');
		*offset++ = PACKED_FIELD(&results[i].action, sizeof(unsigned char), 'C');
	}
	...
	*offset++ = PACKED_FIELD(error, 0, 'S');
    ...
    zbx_free(ehbx_msg);ehbx_msg_alloc=ehbx_msg_offset=0;
    size = message_pack_data(&message, fields, offset - fields);
    zabbix_log(LOG_LEVEL_DEBUG,"%s() packed data: code:6(preproc_test_result) size:%d data(resultNum-[results]-[history]-error):%s", __func__, size, ehbx_msg);
		...
		const zbx_vector_ptr_t *steps)
{
	...
	*offset++ = PACKED_FIELD(&value_type, sizeof(unsigned char), 'I');
	*offset++ = PACKED_FIELD(value, 0, 'S');
	*offset++ = PACKED_FIELD(&ts->sec, sizeof(int), 'I');
	*offset++ = PACKED_FIELD(&ts->ns, sizeof(int), 'I');

	offset += preprocessor_pack_history(offset, history, &history_num);

	*offset++ = PACKED_FIELD(&steps->values_num, sizeof(int), 'I');
    ...
    zbx_free(ehbx_msg);ehbx_msg_alloc=ehbx_msg_offset=0;
    size = message_pack_data(&message, fields, offset - fields);
    zabbix_log(LOG_LEVEL_DEBUG,"%s() packed data: code:5(preproc_test_request) size:%d data(valueType-value-tsSec-tsNs-[history]-stepsNum-[steps]):%s", __func__, size, ehbx_msg);
